                ...
                if (fields.diyname != undefined && fields.beizhu != undefined && fields.diyname.length < 15 && fields.beizhu.length < 30&&fields.shangpinhao!=undefined&&fields.shangpinhao!="") {
                    ...
                    db.find("shangpin", {"shangpinhao": shangpinhao}, function (err, result1) {
                        ...
                        db.find("user", {"username": req.session.username}, function (err, result) {
                                ...
                                if (result1.length > 0&&result1.length==1&&result1[0].shangpinzhuangtai=="2") {
...
exports.addshangpin=function(req,res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        if(files.moxingwenjian1!==undefined) {
            moxingnamearraytemp[1] = files.moxingwenjian1.name;
        }
        ...
        function iterator(i){
            ...
            shijianchuo2d="2d"+shijianchuo2d.getTime()+""+(parseInt(Math.random()*1000));
                                    ...
                                    if (i == piccount - 1) {
                                        ...
                                        function iterator1(j) {
                                            if (j == parseInt(moxingwenjiancount)) {
                                                ...
                                                shijianchuo3d="mx"+shijianchuo3d.getTime()+""+(parseInt(Math.random()*1000));
    ...
    exports.kefuxiugaioneshangpin = function (req, res) {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (shangpinhao !== undefined && shangpinhao !== '') {
                ...
                if(files.moxingwenjian2!==undefined&&files.moxingwenjian2!=="")
                {
                    var moxingwenjian3 = files.moxingwenjian2;
                    moxingnamearraytemp[1] = files.moxingwenjian2.name;
                }
                if(files.moxingwenjian3!==undefined&&files.moxingwenjian3!=="")
                {
                    var moxingwenjian4 = files.moxingwenjian3;
                    moxingnamearraytemp[1] = files.moxingwenjian3.name;
                }
                if(files.moxingwenjian4!==undefined&&files.moxingwenjian4!=="")
                {
                    var moxingwenjian5 = files.moxingwenjian4;
                    moxingnamearraytemp[1] = files.moxingwenjian4.name;
                }
                if(files.moxingwenjian5!==undefined&&files.moxingwenjian5!=="")
                {
                    var moxingwenjian6 = files.moxingwenjian5;
                    moxingnamearraytemp[1] = files.moxingwenjian5.name;
                }
                filearray.push(shangpinpics1);
                filearray.push(shangpinpics2);
                filearray.push(shangpinpics3);
                ...
                file3darray.push(moxingwenjian1);
                file3darray.push(moxingwenjian2);
                file3darray.push(moxingwenjian3);
                file3darray.push(moxingwenjian4);
...
//后台获取商品
exports.get3dModel = function(req,res) {
    db.find("shangpin", {"shangpinzhuangtai":"2"}, function (err, result) {
        res.json({
            "result":result
        });
...
//前台获取商品
exports.getOne3dModel = function(req,res) {
    var shangpinhao = req.query.shangpinhao;
    db.find("shangpin", {"shangpinhao": shangpinhao}, function (err, result) {
        if(err) {
            console.log(err);
        ...
        if(result.length !== 0) {
            var photoPath = [];
            for(var i = 0; i < result[0].patharray.length; i++){
                var pathDiy = result[0].patharray[i];
                photoPath.push(pathDiy);
            }
            var modelPath = result[0].lujing3d + 'model.js';
            // modelPath = modelPath.replace("/", "\\");
            // modelPath = modelPath.replace("/", "\\");
            // modelPath = modelPath.replace("/", "\\");
            // modelPath = modelPath.replace("/", "\\");
            // console.log(path.join(__dirname, '..'+ modelPath));
            // fs.readFile(path.join(__dirname, '..'+ modelPath), function(err,data){
            //     if(err){
            //         res.writeHeader(404,{
...
exports.shangpin3Dyulan= function(req,res) {
    if(req.query!==undefined)
    {
        if (req.query.shangpinhao !== undefined && req.query.shangpinhao !== "") {
            var shangpinhao=req.query.shangpinhao;
            db.find("shangpin", {"shangpinhao": shangpinhao}, function (err, result) {
                ...
                if(result.length!==0) {
                    var str = path.resolve(__dirname, '..');
                    var arg1 = str + '\\public\\python\\convert_obj_three.py';
                    var arg2 = str + result[0].path3darray[0];
                    arg2 = arg2.replace("/", "\\");
                    arg2 = arg2.replace("/", "\\");
                    arg2 = arg2.replace("/", "\\");
                    arg2 = arg2.replace("/", "\\");
                    var arg3 = str + result[0].lujing3d + 'model.js';
                    arg3 = arg3.replace("/", "\\");
                    arg3 = arg3.replace("/", "\\");
                    arg3 = arg3.replace("/", "\\");
                    arg3 = arg3.replace("/", "\\");
                    //console.log('python ' + arg1 + ' -i ' + arg2 + ' -o ' + arg3);
                    exec('python ' + arg1 + ' -i ' + arg2 + ' -o ' + arg3, function(error, stdout, stderr){
                        if(stdout.length >1){
                            res.render("shangpin3Dyulan", {
                                "shangpin":result[0]
                            });
                        } else {
